...
const getEmployeeOverFifty = (employeeData) => new Promise((resolve, reject) => {
  ...
  if(employeeData.length!==0){
   let empList= employeeData.filter((employee)=>(employee.employeeAge>50));
   resolve(empList.length);
  }else{
    reject("Empty Array");
  }
})
...
const getTotalNoOfDaysSinceJoining = (employeeData) => new Promise((resolve, reject) => {
  // Write the code here
  if(employeeData.length!==0){
   let empDOJarray= employeeData.map((employee)=>{
    return Math.floor((new Date().getTime() - new Date(employee.dateOfJoining).getTime()) / (1000 * 3600 * 24))
